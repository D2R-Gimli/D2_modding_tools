...
class NoeBitStream(NoeUnpacker):
	...
	def padToSizeFromCurrentOffset(self, size):
		currentSize = self.getOffset()
		if currentSize < size:
			self.writeBytes(bytearray(size - currentSize))
	def checkEOF(self):
...
class NoeTexture:
	...
	def setPaletteData(self, palColorData, palColorSize, palIndexData, palIndexSize, palFlags):
		self.palColorData = palColorData
		self.palColorSize = palColorSize
		self.palIndexData = palIndexData
		self.palIndexSize = palIndexSize
		self.palFlags = palFlags

	def setHDRData(self, data, dataType, hdrFlags):
		self.hdrData = data
		self.hdrType = dataType
		self.hdrFlags = hdrFlags
		
	def setFrameInfo(self, frameIndex, frameDelay):
...
class NoeKeyFramedValue:
	...
	def __init__(self, time, value):
		...
		self.setFlags(0)
	def setExtraData(self, extraData): #should be a list of floats, needed with some flags and interpolation types
	...
	def setEulerSingleAxisIndex(self, axisIndex):
...
class NoeKeyFramedBone:
	...
	def hasAnyKeys(self):
		return len(self.rotationKeys) > 0 or len(self.translationKeys) > 0 or len(self.scaleKeys) > 0
...
def noeSafeGet(obj, attr, defaultValue = None):
	if hasattr(obj, attr):
