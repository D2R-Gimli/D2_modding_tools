...
==========
Version 4.458:
 - Support for dumping Sega Genesis carts/memory from hardware, see optionalplugins/python/tool_gen_dump.py. There's a comment at the top of the script outlining the setup and hardware you'll need. The protocol also supports remote memory/register writes to deal with any custom hardware you might encounter.
 - COM writes through the Noesis Python API now immediately auto-flush after the write.

Version 4.457:
 - Support for creating Sega CD images from my boot loader and some user code, see optionalplugins/python/tool_gen_makecd.py.
 - Exposed some COM port interface functions to Python: noesis.comGetPortPaths, noesis.comOpenPort, noesis.comClosePort, noesis.comGetInfo, noesis.comSetInfo, noesis.comGetTimeouts, noesis.comSetTimeouts, noesis.comRead, noesis.comWrite
 - Added rapi.deferToOtherHandler.
 - Fixed a mirroring bug in rapi.imageChopTiles8x8.
 - Fixed some typos in the data viewer's extra UV channel list. (thanks Joschka!)
 - Fixed a bug related to loading temporary files during a VR rendering callback.

Version 4.456:
 - Added new options: -imgcroplr, -imgcroprect, -texexidx, -texexcnt
 - Fixed a bug which was causing rapi.imageUntwiddlePS2 with a bit depth of 16 to drop half of the data passed in. Also changed the default block column ordering, and I'm not sure if this is right because I don't have many test cases, so passing -16 will use the old ordering. Let me know if you run into a case where it's necessary.

Version 4.455:
 - Added rapi.imageBlockLinearGOBMaxBlockHeight. Note, however, that many games don't adhere to this standard formula for determining the first mipmap's block height.
 - Minor bug fixes.

Version 4.454:
 - Support for the GTX image format.
 - Added rapi.imageUntileAMDR600 and rapi.imageTileAMDR600.
 - Added an "Allow MMF" setting, accessible through the data viewer.
 - Fixed a bug which was preventing the file associations dialog from being populated.
 - Fixed a couple of ETC encoder bugs.

Version 4.453:
 - Support for Ultima 8 and the Crusader games in optionalplugins/python/fmt_ultima8_flex.py.
 - Support for the CTPK image format.
 - Fixed incorrect byte ordering when loading DICOM images with a multi-byte OB value representation.
 - The following methods are now exposed through the rapi module (most of these already existed as extensions): imageDecodeASTC, imageEncodeASTC, imageDecodeETC, imageEncodeETC, imageUntile1dThin, imageTile1dThin, imageUntileBlockLinearGOB, imageTileBlockLinearGOB, imageBlockLinearGOBBlockHeight, imageDrawText8x8, imageChopTiles8x8, imageGenVDPDrawTile, imageGenVDPExpandPalette, imageGameBoyDrawTile, imageNESDrawTile, imageNESEncodeChar32, imageSNESMode1B0DrawTile, imageSNESMode4B0DrawTile, imageSNESExpandPalette, imageSNESDecodeCharMode1B0, imageSNESEncodeCharMode1B0, imageSNESDecodeCharMode4B0, imageSNESEncodeCharMode4B0, imageJaguarCRYToRGBA, imageJaguarRGBAToCRY, imageJaguarDecodeWidth, imageJaguarClosestWidth, decompLZJag, getLZJagSize.
 - Lots of new functionality in inc_romcom.
 - Implemented some more esoteric compression algorithms through extension methods.
 - Some ROM dump tool scripts have been implemented as image handlers.
 - Added noesis.transformBits, noesis.transformBitsArray, noesis.transformBitsInverse, noesis.transformBitsArrayInverse.
 - Added rapi.lastCheckedPartialChecksum.
 - Added rapi.imageDecodePICA200ETC1, rapi.imageUntilePICA200Raw, rapi.imageTilePICA200Raw, rapi.imageUntilePICA200ETC, rapi.imageTilePICA200ETC.
 - Added rapi.imageResampleNearest.
 - Added rapi.imageArrangeTiles.
 - Added rapi.imageK052109DrawTile.
 - Added rapi.imageOpaqueAlphaRGBA32.
 - Added noesis.BLITFLAG_ALLOWCLIP, noesis.BLITFLAG_ALPHATEST.
 - Added KFBONEFLAG_SCALE_TP.
 - Bumped NOESIS_PLUGINAPI_VERSION.

Version 4.452:
 - 3DS animation support, including correct handling of TCB splines and ease in/out properties.
 - Better 3DS material support, including new functionality to process and composite images in the 3DS loader.
 - Support for 3DS smoothing groups.
 - Support for PSX CD-XA audio/ADPCM and video in optionalplugins/python/fmt_psx_cdxa.py.
 - Added -cuepreservexa (preserves CD-XA sector headers for audio/video/streaming data) and -cuepreserveall (preserves all sector headers) to the cue/bin script.
 - Added Gamebryo support for B-Splines, curve interpolation types, old-style interpolator animations, and more texture/material properties.
 - Added a glTF exporter option to exclude meshes with no-draw materials.
 - Exposed a PSX MDEC implementation via rapi.imagePSXMDEC.
 - Exposed a PSX MPEG-style video decoder via rapi.imagePSXDecodeVideoFrame.
 - New rotation types for the keyframe evaluator: NOEKF_ROTATION_EULER_XYZ_3, NOEKF_ROTATION_EULER_SINGLE, NOEKF_ROTATION_MATRIX_33
 - New interpolation types for the keyframe evaluator: NOEKF_INTERPOLATE_CATMULLROM, NOEKF_INTERPOLATE_TCB, NOEKF_INTERPOLATE_HERMITE, NOEKF_INTERPOLATE_BEZIER, NOEKF_INTERPOLATE_BEZIER_R
 - Exposed keyframe value flags to Python, and added the following flags: KFDFLAG_EASE, KFDFLAG_WRAP_TO_LAST, KFDFLAG_WRAP_TO_FIRST, KFDFLAG_SMOOTHTANGENTS
 - New keyframe bone flags: KFBONEFLAG_NLERP, KFBONEFLAG_NLERP_TP
 - Added rapi.tangentMatricesToTan4.
 - Added NMATFLAG2_BLEND_MAX.
 - Exposed frame delay to Python as a texture property.
 - Added a debug bone structure, see Noesis_AllocBoneDebugInfo.
 - Added Noesis_BlinnPhongToBeckmann and Noesis_BeckmannToBlinnPhong.
 - rpgGetImportantWeights now sorts the output weights by influence and handles some degenerate cases.
 - Bumped the native plugin API version.
 - Fixed skinning not updating correctly when dual quaternion skinning is enabled.
 - Fixed the pUvScaleBias material property, which had been broken for years!
 - Fixed a couple of glTF validation problems.
 - Fixed a GNF crash which could occur with power-of-2 padded images.
 - Fixed an ASTC format header validation bug. (only relevant to the ASTC format, not the ASTC encoder/decoder)
 - Fixed a problem with exception information not being displayed if a Python script managed to cause an exception after successfully loading at least one texture.
 - Fixed the preview renderer crashing on circular dependencies in the model hierarchy.
 - Deprecated -nofslwr, this is now default behavior. (-enablefslwr can be used to revert to old behavior)
 - Other generally more minor edge case and crash fixes.

Version 4.451:
 - First round of x64 fixes.

Version 4.45:
 - Finally finished that x64 support! x86 (Noesis.exe) and x64 (Noesis64.exe) builds coexist in the same distribution. If you encounter any x64-specific problems, and I'm sure there are still a lot, let me know.
 - Support for Castlevania: Resurrection meshes, animations, levels, and collision data in optionalplugins/python/fmt_cvresurrection.py.
 - Updated to Kerilk's latest Bayonetta/PG plugin code, x64 support is included.
 - Exposed support for per-texture LOD bias in the data viewer. The pixel picker will also obey the selected LOD for convenient inspection of specific mipmaps.
 - Several md5mesh/md5anim additions and fixes.
 - Added NMATFLAG2_PBR_METAL_SPECBLUE.
 - Exposed KFBONEFLAG_* and KFANIMFLAG_* flags to Python.
 - Increased the number of message handlers for tool menu items, since this is becoming a more popular feature!
 - Made the display of additional plugin tool menu items an option in the tools menu. This also affects plugin context menu items.
 - Fixed a bug which was causing Python exceptions during log flushes.
 - Fixed a circular dependency problem in pluginshare.h.

Version 4.442:
 - Added EXR import/export support.
 - HDR texture data is now exposed to Python.
 - Added support for preserving more HDR data types when exporting FourCC-DX10 DDS files.
 - When importing FourCC-DX10 DDS files, linear texture flags will now be added to textures sourced from non-sRGB data.
 - Added a -ddsnohdr option, which prevents HDR formats from being used when exporting DDS files.
 - Added rapi.imageGetTexRGBAFloat.
 - Added rapi.scaleAndBiasPackedFloats.
 - Fixed Radiance HDR exporter potentially performing a linear transform on already-linear data.

Version 4.441:
 - Some new image/normal processing functions: rapi.imageExpandU8Normals, rapi.imagePackU8Normals, rapi.imagePrefilterRoughness, rapi.imageTransformTangentSpaceNormals
 - Fixed a bug in fmt_nbajamsrc_img.py which was preventing some images in NARC (and possibly others) from loading.
 - New utility functions in inc_romcom.
 - Fixed a bug with RAPI module instance tracking, which was the cause of many mysterious crashes.

Version 4.44:
 - New script, optionalplugins/python/fmt_powerupbaseball_img.py.
 - Fixed a recently introduced problem with non-archive types sharing an extension with archive types.

Version 4.439:
 - Lots of new Atari Jaguar functionality, see inc_atarijaguar.py.
 - Model/image support for Cybermorph for the Atari Jaguar. See optionalplugins/python/fmt_atarijaguar_cybermorph.py.
 - Atari Jaguar CRY image support. See optionalplugins/python/fmt_atarijaguar_cry.py.
 - Lots of under-the-hood SNES/Genesis functionality.
 - Support for a few new compression types.
 - Fixed universal debugger's disassembler not correctly tracking the PC register if it happens to be the last register in the list.
 - VR menu/control adjustments.
 - Fixed a viewport resize bug which was introduced in 4.435.

Version 4.438:
 - Fixed an issue which was causing high-precision timers to break on some systems.

Version 4.437:
 - Added a new VR menu, which can be extended via plugin/script. tool_vrbrowser.py is included under optionalplugins, it provides a file browser through the VR menu.
 - Added a "VR scene unit to meter scale" option to the data viewer. By default, Noesis creates a scene unit scale based on the size of a given model. This option will override the size-based scale. For a game with units already in meters, such as FF15, the correct value is 1.0. For a game with units are in centimeters, like most UE4 games, the correct value is 0.01.
 - Added submenu support for custom tools, for examples see the use cases of noesis.setToolSubMenuName in the existing scripts.
 - Added a "VR head to background scale" option to the data viewer. This scales head motion relative to the background shell.
 - Added tool_sethdrmode.py under optionalplugins. This adds a tools menu option to automatically configure Noesis to render in HDR, use environment-based backgrounds, and so on.
 - Fixed VR crash when viewing a model with no material data.
 - Added more acceleration options for VR controls.
 - Lots of PBR fastpath fixes and a few other random bug fixes.
 - Quite a few API additions, check the updated header for details.

Version 4.436:
 - Added a "force PBR fastpath" option to the data viewer. This will force everything down the PBR material pipe, disable all CPU-based transform features, mandate static geometry, and perform limited transform functionality (basic skinning) on the GPU. This mode is incompatible with many existing material features, but it's really fast. When this mode is enabled, many material properties will only be visibly updated when resetting the view. (middle mouse or F12)

Version 4.435:
 - Added MSAA options to the data viewer.

Version 4.434:
 - Early VR support. For instructions, see optionalplugins/vr/vr.txt.
 - Fixed some BC5-BC7 encoding problems.
...
Version 4.431:
 - Support for Kingpin MDX files.
 - Moved crunch support out of noesis_misc, it's now a standalone DLL that comes in optionalplugins. (its existence creates too much general overhead to justify keeping it in stock)
 - Fixed Quake 2 BSP loader not matching materials to textures correctly with certain external image loaders.
 - Fixed some bugs introduced by the structural changes in the last release.

Version 4.43:
 - Early support for A Plague Tale: Innocence, including archives, models, map parts, and animations.
 - Major overhaul to the way module instancing is handled internally. This could lead to some instability in this release, let me know if you encounter specific crashes or broken behavior which occurs in 4.43 but not 4.428.
 - New script in optionalplugins to discover data in compiled Animal Forest source object files.
 - Removed the sad, neglected Bayonetta plugin and incorporated the much more complete and actively maintained version by Brice Videau. This version also comes with support for animations and additional games/platforms. Source code for the plugin is available at: https://github.com/Kerilk/noesis_bayonetta_pc
 - Added support for Animated IFF/ILBM compression mode 75, a more obscure mode only used by AnimBrushes and only in some versions of DeluxePaint.
 - New script in optionalplugins which supports the source image format used in many Midway games, including Smash TV, Judge Dredd, and NBA Jam.
 - New script in optionalplugins for Amiga ADF images.
 - Support for 4-bit paletted Dreamcast PVR textures.
 - Added support for big endian files to the ELF loader, and fixed some bugs with relocatable object handling.
 - Various functionality added to the Python-based N64 displaylist interpreter.
 - Other N64-related functionality including simulated TMEM reads/writes.
 - Added an "-animfilter" option, this specifies one or more animations by name in order to reduce the exported set.
 - Added a "-logfile" option to direct all output for the given module to a file.
 - Added a "-nopreviewtex" option to disable automatic preview texture loading.
 - Added a "force contiguous" option for the FAT script to assist in recovering deleted/corrupted files.
 - Added FBX "don't skin to world" and "force affine node transforms" options.
 - Added "roughness from normal map blue channel" and "occlusion from green" material flags.
 - Added a KFBONE_ADDITIVE flag to the keyframe interpreter.
 - "Safe types" now defaults to enabled, in order to take advantage of various load time improvements.
 - Lots of bug/crash fixes and speed improvements.
...
Brice Videau:
 - Created the new and improved Bayonetta/PG plugin which is now included with Noesis! Source code for the plugin is available at: https://github.com/Kerilk/noesis_bayonetta_pc
